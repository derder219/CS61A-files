...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        return Procedure.eval_call(scheme_eval(first, env), rest, env)
        # END PROBLEM 5
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        # BEGIN PROBLEM 11
        def unpacking(lst1, lst2):
            if lst1 == nil and lst2 == nil:
                return child
            elif len(lst1) != len(lst2):
                raise SchemeError
            else:
                child.bindings[lst1.first] = lst2.first
                unpacking(lst1.second, lst2.second)
        unpacking(formals, vals)
        # END PROBLEM 11
        return child
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        #self.operator.eval(env).apply([i.eval(env) for i in self.operands])
        operands_eval = operands.map(lambda x: scheme_eval(x, env))
        return scheme_apply(self, operands_eval, env)
        # END PROBLEM 5
...
def make_let_frame(bindings, env):
    ...
    while len(bindings) > 0:
        ...
        scheme_formals = Pair(bindings.first.first, scheme_formals)
        scheme_values = Pair(scheme_eval(bindings.first.second.first, env), scheme_values)
        bindings = bindings.second
    check_formals(scheme_formals)
    return env.make_child_frame(scheme_formals, scheme_values)
    # END PROBLEM 15
