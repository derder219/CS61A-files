...
def distance(city1, city2):
    ...
    5.0
    """
    x1, x2, y1, y2 = get_lat(city1), get_lat(city2), get_lon(city1), get_lon(city2)
...
def closer_city(lat, lon, city1, city2):
    ...
    'Bucharest'
    """
    subject = ('place', lat, lon)
    if distance(subject, city1) > distance(subject, city2):
        return city2[0]
    else:
        return city1[0]
...
def put_piece(board, max_rows, column, player):
    ...
    while r > -1:
        if get_piece(board, r, column) == '-':
            new_row = replace_elem(board[r], column, player)
            return (r, replace_elem(board, r, new_row))
        r -= 1
    return (r, board)
